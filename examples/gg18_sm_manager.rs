...
use std::collections::HashMap;
use std::fs;
use std::sync::{Arc, RwLock}; // Arc 추가

use rocket::serde::json::Json;
use rocket::{get, post, routes, State}; // get 추가
use uuid::Uuid;
...
#[get("/")]
fn index() -> &'static str {
    "Welcome to GG18 SM Manager API"
}

#[post("/get", format = "json", data = "<request>")]
fn get(
    db_mtx: &State<Arc<RwLock<HashMap<Key, String>>>>, // Arc 사용
    request: Json<Index>,
) -> Json<Result<Entry, ()>> {
    let index: Index = request.0;
    let hm = db_mtx.read().unwrap();
...
#[post("/set", format = "json", data = "<request>")]
fn set(
    db_mtx: &State<Arc<RwLock<HashMap<Key, String>>>>,
    request: Json<Entry>,
) -> Json<Result<(), ()>> {
    let entry: Entry = request.0;
    let mut hm = db_mtx.write().unwrap();
...
async fn main() {
    let db: HashMap<Key, String> = HashMap::new();
    let db_mtx = Arc::new(RwLock::new(db));

    rocket::build()
        .mount("/", routes![index, get, set])
        .manage(db_mtx)
        .launch()
        .await
        .unwrap();
